...
def MatriceGainsModifiee(nbPierresJ1, nbPierresJ2,positionTroll,nbCases,matrice) : # remplissage de la matrice de gains
    positionTrollInitiale = positionTroll - (nbCases//2 + 1 ) # 1 = -3 = Chez le J1, 4 = 0 = Au Milieu, 7 = 3 = Chez le J2 Pour nbCases = 7
    for i in range (nbPierresJ1) :
        for j in range (nbPierresJ2) : # i = nb pierres restantes au J1, j = nb pierres restantes au J2
            t = positionTrollInitiale
...
def ProbasPourUneSituationDeJeuModifiee(x=15,y=15,troll=4,cases=7) : # Executions de debogage, permet de voir les distributions de probabilite a partir d'une matrice
    #g = [
    #    [31/6,-1,-2,-3],
    #    [-1,31/6,-1,-2],
    #    [-2,-1,31/6,-1],
    ...
    MatriceGainsModifiee(x,y,troll,cases,matrice)
    matTmp = []
    for i in range(len(matrice)) :
        col = []
                ...
                if t == 0:
                    ...
                    for iTmp in range(i):
                        ...
                        for jTmp in range(j):
                            col.append(matrice[iTmp][jTmp])
                        matTmp.append(col)
                    matrice[i][j] =  ValeurGainsMatriceQuadratique(len(matTmp),len(matTmp[0]),matTmp) # si les joueurs ont le même nombre de pierres, le cas est trivial si le troll est au milieu (pas d'avantage pour un joueur). Sinon, ce n'est plus un cas trivial.
            else : # Sinon
                matTmp = []
                for iTmp in range(i):
                    col = []
