...
def StrategiePrudenteModifiee(nbPierresCourant,nbPierresCourantAdversaire,nbCases,positionTroll) :
    #Calcul de la matrice de gains
    if nbPierresCourant > 1 : # Si il ne reste qu'une seule pierre, on peut retourner 1 directement.
        matrice = []
        for i in range(nbPierresCourant) :
            ...
            for j in range(nbPierresCourantAdversaire) :
                col.append(float("inf"))
            matrice.append(col) # creation d'une matrice qui va stocker les gains.
        simplex.MatriceGainsModifiee(nbPierresCourant,nbPierresCourantAdversaire,positionTroll,nbCases,matrice) # calcul de la matrice de gains en fonction du nombre de pierres lancees
        matTmp = []
